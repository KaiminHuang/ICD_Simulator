...
package body HRM is
   ...
   begin
      Hrm.IsOn := False;
      Hrm.Rate := Measures.BPM'First; -- this return the smallest rate of the range
   end Init;
   ...
   procedure GetRate(Hrm : in HRMType; Rate : out Measures.BPM) is
   begin
      if Hrm.IsOn then 
         Rate := Hrm.Rate;
      else 
   ...
   begin
      if Hrm.IsOn then
         ...
         Hrm.Rate := Measures.LimitBPM(RandomNumber.UniformIntegerWithError(Hrm.Rate, Error));
      else
